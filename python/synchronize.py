...
def main():
    ...
    sensor_list = []
    try:
        # First of all, create the client that will send the requests, assume port is 2000
        ...
        while True:
            ...
            # As the queue is blocking, we will wait in queue.get() methods
            # until all the information is processed and we continue with the next frame.
            try:
                for i in range(0, len(sensor_list)):
                    s_frame = sensor_queue.get(True, 1.0)
                    print("    Frame: %s   Sensor: %s" % (s_frame[0], s_frame[1]))
